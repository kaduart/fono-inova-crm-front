...
export default function MarketingDashboard() {
  ...
  // 🧪 Tracking de visualização do post A/B
  useEffect(() => {
    if (previewModal?.open && previewModal.post?._id) {
      trackGmbPostView(previewModal.post._id);
    }
  }, [previewModal?.open, previewModal?.post?._id]);
          ...
          <div className="bg-white rounded-2xl max-w-lg w-full max-h-[90vh] overflow-hidden shadow-xl" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-6 py-4 border-b border-blue-100 bg-blue-50/30">
              <div>
                <div className="flex items-center gap-2">
                  ...
                  {previewModal.post.metadata?.abVariant && (
                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                      previewModal.post.metadata.abVariant === 'A'
                        ? 'bg-blue-100 text-blue-700'
                        : 'bg-orange-100 text-orange-700'
                    }`}>
                      Variante {previewModal.post.metadata.abVariant}
                    </span>
                  )}
                </div>
            ...
            <div className="px-6 py-4 border-t border-gray-100 bg-gray-50/50 flex flex-wrap items-center justify-between gap-3">
              <div className="flex flex-wrap items-center gap-2">
                {previewModal.post.metadata?.abVariant && (
                  <button
                    onClick={() => openGmbWhatsApp(previewModal.post)}
                    className="inline-flex items-center gap-1.5 px-4 py-2 text-sm text-white bg-green-600 rounded-lg hover:bg-green-700 transition-colors"
                  >
